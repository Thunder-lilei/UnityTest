...
namespace BiuBiu.Data
{
    ...
    {
        Normal, // 普通敌人（普通丧尸/投掷丧尸/门板丧尸）
        Elite,  // 精英（强化门板丧尸，3:00 起每 180s 一只）
        Boss    // Boss（森林大蜘蛛，5:00 起每 300s 一只，一只比一只强）
    }

    /// <summary>普通敌人攻击方式（数值文档 5.1「攻击」列）</summary>
    public enum EnemyAttackType
    {
        MeleeSingle, // 近战单点（普通丧尸：贴身判定）
        RangedLine,  // 远程直线（投掷丧尸：直线弹丸）
        ArcSweep     // 范围横扫（门板丧尸 120°/半径 2.0；精英横扫 120°/半径 2.5）
    }
    ...
    {
        [Header("标识")]
        [Tooltip("配置标识（normal_zombie / thrower_zombie / door_zombie / elite / boss_spider），代码与文档引用键")]
        public string enemyId;

        [Tooltip("显示名（Hierarchy 可读性与提示文案用）")]
        public string displayName;

        [Tooltip("敌人类型：普通（配额内）/ 精英（独立定时）/ Boss（独立定时）")]
        ...
        [Tooltip("移速（tile/s，v3.0 移速×2）：普通丧尸 3.6 / 投掷 3.0 / 门板 2.4 / 精英 3.0 / Boss 1.6")]
        ...
        [Tooltip("横扫角度（度，仅 ArcSweep 用）：门板 120 / 精英 120")]
        public float sweepAngle = 120f;

        [Tooltip("弹速（tile/s，仅 RangedLine 用；投掷丧尸 6.0，其余=0）")]
        public float projectileSpeed;

        [Tooltip("类型解锁时间（秒，仅普通敌人用；v2.4 起三丧尸开局混合=全 0）：精英/Boss=0（走独立定时器）")]
