...
- **敌人四型**：近战（扇形直冲）、远程（直线弹）、精英（横扫+冲撞）、Boss（5:00 解锁，环形+直线弹）；近战含扇形型与横扫型两种
- **波次成长**：每轮结束自动微增移速与攻击力
- **操作**：`WASD` 移动 · 左键蓄力射击 · 空格闪避翻滚 · `ESC` 暂停 · `F3` 开发者模式无敌
- **反馈（Game Feel）**：受击闪白、命中微后仰、击碎（同色像素碎片爆发）、镜头震屏/慢动作（hitstop）、尸体与血迹池上限、**玩家受击屏幕红边**、**发射后坐力（三档递进）**、**弹丸拖尾（三档差异，红档粗光带）**、**蓄力音效**
...
- 🚧 M3 系统扩展（Boss / 精英 / 近战远程差异化灰盒表现）
- 🚧 M4 元游戏与打磨（音频部分接入：蓄力音已接，其余事件音待补充；成就/统计/开始页待做）
