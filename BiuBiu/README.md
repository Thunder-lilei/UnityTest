...
- **敌人三型**：近战（横扫，120° 大扇形，前摇 0.9s）、远程（直线弹）、Boss（第 5、10 轮登场，八方向扇形横扫 240°·r4 + 直线冲撞）；精英（第 3 轮起每轮 +1，紫色，八方向投掷封锁，投掷距离 = 普通远程 ×2）。近战/精英攻击清晰的前摇蓄力窗口供翻滚闪避
- **波次成长**：每轮结束自动微增移速与攻击力
...
- **反馈（Game Feel）**：受击闪白、命中微后仰、击碎（同色像素碎片爆发）、镜头震屏/慢动作（hitstop）、尸体与血迹池上限、**玩家受击屏幕红边**、**发射后坐力（三档递进）**、**弹丸拖尾（三档差异，红档粗光带）**、**蓄力音效 + 闪避音 + 三档发射音**、**撞墙火花/碎墙反馈**
...
- 🚧 M4 元游戏与打磨（音频已接入蓄力/闪避/三档发射/碎石；开始页/成就系统等元游戏待做）
