...
> 项目路径：`c:\AIProject\UnityTestDemo\Test\`
> 引擎版本：
> 开始日期：2026-07-10
...
**解决方案**：
1. **导入中文字体文件**：将系统字体（如 `C:\Windows\Fonts\msyh.ttc` 微软雅黑）复制到 `Assets/TextMesh Pro/Fonts/` 目录下
...
**注意事项**：
- 可选的中文字体：`msyh.ttc`（微软雅黑）、`simhei.ttf`（黑体）、`simkai.ttf`（楷体）等，均在 `C:\Windows\Fonts\` 下
