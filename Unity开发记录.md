...
### Unity 编辑器汉化（简体中文语言包安装）

团结引擎 (Tuanjie / Unity 中国版) 2022.3.62t11 安装简体中文语言包的方法：

**方法一：检查内置支持（优先尝试）**

打开 Edit > Preferences，查看左侧是否有 Language 选项卡。如果有，下拉选「中文」，重启 Editor 即可。

**方法二：通过团结 Hub 添加语言模块（本次使用的方法）**

1. 打开团结 Hub
2. 左侧选「安装」
3. 找到 Tuanjie 2022.3.62t11 版本
4. 点击右侧齿轮图标 ⚙️ > 添加模块 (Add Modules)
5. 在模块列表中找到 Language packs (Preview)，勾选简体中文
6. 点击安装，等待安装完成
7. 回到 Editor，Edit > Preferences > Language 切换为 Chinese，重启

**方法三：手动下载语言包**

如果 Hub 里没有语言模块，去 Unity 中国官网或 Tuanjie 下载页搜索对应版本的 Language Pack，下载后放到 Tuanjie 安装目录的 `Editor\Data\Localization` 文件夹中，重启 Editor 后切换语言。

**注意事项**：

| 项目 | 说明 |
|---|---|
| 切换后需重启 | 语言切换后必须重启 Editor 才能生效 |
| 覆盖范围 | 菜单、Inspector、偏好设置等大部分界面汉化 |
| 第三方插件 | 部分第三方插件可能仍为英文，不受语言包影响 |
...
## Lua 在 Unity 开发中的应用与热更

### Lua 在 Unity 中的典型用途

| 用途 | 说明 |
|---|---|
| **热更新（Hotfix）** | iOS 不允许动态下载可执行代码，通过 Lua 解释器运行逻辑，实现不重新发版即可修复 Bug 或更新逻辑 |
| **业务逻辑层** | 核心游戏逻辑（战斗、任务、UI 逻辑）用 Lua 编写，C# 负责底层引擎功能，逻辑可热更 |
| **配置与数据驱动** | 用 Lua 表替代 JSON/XML 做配置，支持逻辑表达式，读取后直接是 Lua 对象 |
| **脚本化 AI / 行为树** | NPC 行为逻辑用 Lua 编写，便于快速迭代，策划可直接修改 |
| **跨引擎逻辑复用** | Lua 代码与引擎无关，可在 Unity、Cocos2d-x、服务端之间共享 |
| **插件 / Mod 系统** | 让玩家通过 Lua 编写 Mod，扩展游戏内容而无需暴露 C# 源码 |

### 常见 Lua 框架

| 框架 | 说明 |
|---|---|
| **xLua**（腾讯开源） | 国内最流行，支持 C#↔Lua 双向调用，热补丁能力最强 |
| **ToLua#** | 较早的方案，性能好但生态不如 xLua |
| **SLua** | 另一个 Lua 绑定方案 |

### 热更新的本质目的

热更不是"游戏运行时实时替换代码"，而是**不让用户重新去应用商店下载新版本**。

典型场景：
1. 线上发现 Bug → 发 Lua 补丁包，客户端启动时下载替换 → 修复
2. 运营活动 / 数值调整（双倍经验、新升级选项）→ 更新 Lua 配置 → 生效
3. iOS 审核要几天，紧急 Bug 等不起 → Lua 热更绕过审核

**核心价值：绕过平台审核流程，快速修复线上问题。**

Lua 热更是**手游时代**的产物，主要为了应对 iOS App Store 审核慢、Android 渠道碎片化的问题。对于 Windows PC 单机游戏，没有审核流程，直接发新 exe 即可，热更意义不大。

### 为什么不用 Python 做热更

两者都能做"脚本化逻辑"，但 Python 在游戏热更场景中几乎不被采用，原因如下：

#### 1. 定位根本不同

| | Lua | Python |
|---|---|---|
| 设计初衷 | **嵌入式脚本语言**，给 C/C++ 当扩展层 | 通用编程语言，独立运行 |
| 典型宿主 | 游戏引擎内部嵌入 | 独立进程 / 后端服务 |

Lua 从诞生第一天就是设计来被嵌入 C 程序中的，API 极其简洁。Python 虽然也能嵌入，但远没有 Lua 自然。

#### 2. 性能差距

| 指标 | Lua | Python | 差距 |
|---|---|---|---|
| 解释器体积 | ~200KB | ~30MB+ | 150x |
| 内存占用 | 极低 | 高 | 数倍~数十倍 |
| 执行速度 | 快（LuaJIT 接近原生 C） | 慢 | 3-10x |
| GC 停顿 | 增量式，可控 | 分代 GC，停顿大 | — |

游戏中每帧都要执行大量脚本逻辑（AI、战斗判定、UI 刷新），Python 的性能在此场景下不可接受。
...
env.DoString("print('hello')");
env.DoFile("config.lua");
```

**Python 嵌入需要：**
- 引入 IronPython（.NET）或 Pythonnet
- IronPython 已停止主流支持，Pythonnet 对 Unity 兼容性差
- 或通过子进程调用 Python，但这就不是"热更"而是"跨进程通信"了
- Python 运行时依赖庞大的标准库（~30MB），打包进游戏体极不合理

#### 4. GC 问题

游戏要求**稳定帧率**（60fps = 每帧 16ms）。Python 的 GC 在大规模对象分配时会产生明显停顿，导致掉帧。Lua 的 GC 是**增量式**的，可以控制每帧步进量，不会造成可感知的卡顿。

#### 5. 线程模型

- **Lua**：协程原生支持，单线程内协作式调度，与 Unity 主线程完美匹配
- **Python**：GIL（全局解释器锁），多线程形同虚设，反而增加复杂度
...
| **xLua**（腾讯） | 王者荣耀等大量腾讯游戏 |
| **ToLua** | 众多国内手游 |
| **Lua + Cocos2d-x** | 早期手游标配 |
| **魔兽世界** | UI 完全用 Lua |
| **Garry's Mod** | 全部游戏逻辑用 Lua |

Python 在游戏领域几乎没有成功的热更案例。
...
|---|---|---|
| 运行时轻量 | ✅ ~200KB | ❌ ~30MB+ |
| 与宿主语言双向交互简洁 | ✅ C#↔Lua 调用成本低 | ❌ 嵌入复杂 |
| GC 不卡帧 | ✅ 增量式 GC | ❌ 分代 GC 停顿大 |
| 沙箱安全（裁剪危险 API） | ✅ 可裁剪 | ❌ 标准库庞大 |
| 打包体积小 | ✅ 不显著增加包体 | ❌ 显著增大包体 |

### 本项目的适用性分析

本项目是 Windows PC 单机生存游戏，单场景、无服务端、无持久化存档。在此前提下**不需要 Lua 热更**——Windows 上直接发新 exe 即可，没有审核流程。

如果排除热更需求，Lua 剩下的价值主要是**数据驱动配置**，但 JSON / ScriptableObject 同样能做，且不需要引入 Lua 运行时。

**结论**：对于当前项目，引入 Lua 的实际收益很低。若未来要做成手游并上线运营，再考虑 Lua 热更方案。
